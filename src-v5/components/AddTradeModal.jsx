...
function AddTradeModal({
  ...
  const amountTooltip = 'How many units you bought or sold. Must be greater than zero.'
  const priceTooltip = 'Price per unit at the time of the trade. Must be greater than zero.'
  const orderTooltip = 'Choose Buy when entering, Sell when exiting.'
  const tagTooltip = 'Optional strategy label to organize trades.'
  const formErrorId = 'trade-form-error'
  ...
  const assetLabel = formData.asset.trim().toUpperCase()
  const sellHelperText =
    formData.order === 'Sell'
      ? `Available to sell: ${sellableAmount ?? 0}${assetLabel ? ` ${assetLabel}` : ''}.`
      : 'Buy trades increase holdings. Sell trades reduce holdings.'
