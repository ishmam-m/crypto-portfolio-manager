export const TRADES_STORAGE_KEY = 'trades'
export const HOLDINGS_STORAGE_KEY = 'holdings'
export const TAG_OPTIONS = ['Short-Term', 'Long-Term']
const ORDER_OPTIONS = ['Buy', 'Sell']

export const INITIAL_FORM = {
  asset: '',
  amount: '',
  price: '',
  order: 'Buy',
  tag: TAG_OPTIONS[0],
}

const TRADES = [
  {
    id: crypto.randomUUID(),
    asset: 'BTC',
    amount: 0.015,
    price: 68000,
    order: 'Buy',
    tag: 'Long-Term',
    createdAt: '2026-02-22T05:00:00.000Z',
  },
  {
    id: crypto.randomUUID(),
    asset: 'ETH',
    amount: 0.42,
    price: 1960,
    order: 'Buy',
    tag: 'Short-Term',
    createdAt: '2026-02-21T05:00:00.000Z',
  },
  {
    id: crypto.randomUUID(),
    asset: 'SOL',
    amount: 2.5,
    price: 82.5,
    order: 'Sell',
    tag: 'Short-Term',
    createdAt: '2026-02-20T05:00:00.000Z',
  },
]

const HOLDINGS = [
  { asset: 'BTC', amount: 0.17, avgCost: 63100, currentPrice: 68640 },
  { asset: 'ETH', amount: 2.8, avgCost: 2810, currentPrice: 3210 },
  { asset: 'SOL', amount: 19, avgCost: 128, currentPrice: 146.3 },
]

export const moneyFormatter = new Intl.NumberFormat('en-US', {
  style: 'currency',
  currency: 'USD',
  maximumFractionDigits: 2,
})

export const compactMoneyFormatter = new Intl.NumberFormat('en-US', {
  style: 'currency',
  currency: 'USD',
  notation: 'compact',
  maximumFractionDigits: 2,
})

export function formatTradeTimestamp(timestamp) {
  if (!timestamp) {
    return 'No timestamp'
  }

  const parsed = new Date(timestamp)

  if (Number.isNaN(parsed.getTime())) {
    return 'No timestamp'
  }

  return parsed.toLocaleString('en-US', {
    month: 'short',
    day: 'numeric',
    hour: 'numeric',
    minute: '2-digit',
  })
}

export function renderTradeLine(trade) {
  if (!Number.isFinite(trade.amount) || !Number.isFinite(trade.price) || !trade.asset) {
    return trade.summary ?? 'Trade entry'
  }

  return `${trade.asset} ${trade.order} ${trade.amount} @ ${moneyFormatter.format(trade.price)}`
}

function normalizeTrade(rawTrade) {
  if (!rawTrade || typeof rawTrade !== 'object') {
    return null
  }

  return {
    id: rawTrade.id ?? crypto.randomUUID(),
    asset: typeof rawTrade.asset === 'string' ? rawTrade.asset.toUpperCase() : null,
    amount: Number(rawTrade.amount),
    price: Number(rawTrade.price),
    order: rawTrade.order === 'Buy' ? 'Buy' : 'Sell',
    tag: typeof rawTrade.tag === 'string' ? rawTrade.tag : TAG_OPTIONS[0],
    createdAt: rawTrade.createdAt ?? null,
    summary: typeof rawTrade.summary === 'string' ? rawTrade.summary : null,
  }
}

function normalizeHolding(rawHolding) {
  if (!rawHolding || typeof rawHolding !== 'object') {
    return null
  }

  const asset = typeof rawHolding.asset === 'string' ? rawHolding.asset.toUpperCase().trim() : ''
  const amount = Number(rawHolding.amount)
  const avgCost = Number(rawHolding.avgCost)
  const currentPrice = Number(rawHolding.currentPrice)

  if (!asset || !Number.isFinite(amount) || amount <= 0) {
    return null
  }

  return {
    asset,
    amount,
    avgCost: Number.isFinite(avgCost) && avgCost > 0 ? avgCost : 0,
    currentPrice: Number.isFinite(currentPrice) && currentPrice > 0 ? currentPrice : 0,
  }
}

function parseStoredCollection(storedValue, normalizer) {
  if (!storedValue) {
    return []
  }

  const parsed = JSON.parse(storedValue)
  return parsed.map(normalizer).filter(Boolean)
}

export function loadTrades() {
  try {
    const stored = localStorage.getItem(TRADES_STORAGE_KEY)
    const normalized = parseStoredCollection(stored, normalizeTrade)
    return normalized.length ? normalized : TRADES
  } catch {
    return TRADES
  }
}

export function loadHoldings() {
  try {
    const stored = localStorage.getItem(HOLDINGS_STORAGE_KEY)
    const normalized = parseStoredCollection(stored, normalizeHolding)
    return normalized.length ? normalized : HOLDINGS
  } catch {
    return HOLDINGS
  }
}

export function createTradeFromForm(formData) {
  if (!formData || typeof formData !== 'object') {
    return null
  }

  const asset = formData.asset.trim().toUpperCase()
  const amount = Number(formData.amount)
  const price = Number(formData.price)
  const order = typeof formData.order === 'string' ? formData.order : ''
  const tag = typeof formData.tag === 'string' ? formData.tag : ''

  if (!asset) {
    return null
  }

  if (!Number.isFinite(amount) || !Number.isFinite(price) || amount <= 0 || price <= 0) {
    return null
  }

  if (!ORDER_OPTIONS.includes(order) || !TAG_OPTIONS.includes(tag)) {
    return null
  }

  return {
    id: crypto.randomUUID(),
    asset,
    amount,
    price,
    order,
    tag,
    createdAt: new Date().toISOString(),
  }
}

export function tradeToFormData(trade) {
  return {
    asset: trade.asset ?? '',
    amount: trade.amount ?? '',
    price: trade.price ?? '',
    order: trade.order === 'Sell' ? 'Sell' : 'Buy',
    tag: typeof trade.tag === 'string' ? trade.tag : TAG_OPTIONS[0],
  }
}

export function applyTradeToHoldings(currentHoldings, trade) {
  const holdings = [...currentHoldings]
  const holdingIndex = holdings.findIndex((holding) => holding.asset === trade.asset)
  const isBuy = trade.order === 'Buy'

  if (isBuy) {
    if (holdingIndex === -1) {
      return [
        ...holdings,
        {
          asset: trade.asset,
          amount: trade.amount,
          avgCost: trade.price,
          currentPrice: trade.price,
        },
      ]
    }

    const existing = holdings[holdingIndex]
    const nextAmount = existing.amount + trade.amount
    const nextAvgCost = (existing.amount * existing.avgCost + trade.amount * trade.price) / nextAmount

    holdings[holdingIndex] = {
      ...existing,
      amount: nextAmount,
      avgCost: nextAvgCost,
      currentPrice: trade.price,
    }

    return holdings
  }

  if (holdingIndex === -1) {
    return holdings
  }

  const existing = holdings[holdingIndex]
  const nextAmount = Math.max(existing.amount - trade.amount, 0)

  if (nextAmount === 0) {
    return holdings.filter((holding) => holding.asset !== trade.asset)
  }

  holdings[holdingIndex] = {
    ...existing,
    amount: nextAmount,
    currentPrice: trade.price,
  }

  return holdings
}

export function removeTradeFromHoldings(currentHoldings, trade) {
  if (!trade?.asset || !Number.isFinite(trade?.amount) || !Number.isFinite(trade?.price)) {
    return currentHoldings
  }

  const holdings = [...currentHoldings]
  const holdingIndex = holdings.findIndex((holding) => holding.asset === trade.asset)
  const isBuy = trade.order === 'Buy'

  if (isBuy) {
    if (holdingIndex === -1) {
      return holdings
    }

    const existing = holdings[holdingIndex]
    const nextAmount = Math.max(existing.amount - trade.amount, 0)

    if (nextAmount === 0) {
      return holdings.filter((holding) => holding.asset !== trade.asset)
    }

    const recoveredCostBasis = existing.amount * existing.avgCost - trade.amount * trade.price
    const recoveredAvgCost = recoveredCostBasis / nextAmount

    holdings[holdingIndex] = {
      ...existing,
      amount: nextAmount,
      avgCost:
        Number.isFinite(recoveredAvgCost) && recoveredAvgCost >= 0
          ? recoveredAvgCost
          : existing.avgCost,
    }

    return holdings
  }

  if (holdingIndex === -1) {
    return [
      ...holdings,
      {
        asset: trade.asset,
        amount: trade.amount,
        avgCost: trade.price,
        currentPrice: trade.price,
      },
    ]
  }

  const existing = holdings[holdingIndex]
  holdings[holdingIndex] = {
    ...existing,
    amount: existing.amount + trade.amount,
  }

  return holdings
}

export function calculatePortfolioStats(holdings) {
  const { value, costBasis } = holdings.reduce(
    (accumulator, holding) => {
      accumulator.value += holding.amount * holding.currentPrice
      accumulator.costBasis += holding.amount * holding.avgCost
      return accumulator
    },
    { value: 0, costBasis: 0 },
  )

  const pnl = value - costBasis
  const pnlPercent = costBasis ? (pnl / costBasis) * 100 : 0

  return { value, costBasis, pnl, pnlPercent }
}

export function calculateAllocations(holdings) {
  const totalValue = holdings.reduce((total, holding) => total + holding.amount * holding.currentPrice, 0)

  return holdings
    .map((holding) => {
      const value = holding.amount * holding.currentPrice
      const percentage = totalValue > 0 ? (value / totalValue) * 100 : 0

      return {
        ...holding,
        value,
        percentage,
      }
    })
    .sort((a, b) => b.value - a.value)
}
