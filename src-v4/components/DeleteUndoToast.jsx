...
function DeleteUndoToast({ deletedTrade, onUndo, onDismiss, renderTradeLine }) {
  ...
  return createPortal(
    <aside
      role="status"
      aria-live="polite"
      className="fixed inset-x-4 bottom-4 z-[70] mx-auto w-full max-w-lg rounded-xl border border-slate-700 bg-slate-900/95 p-4 shadow-2xl"
    >
      <p className="text-sm text-slate-100">Trade deleted: {renderTradeLine(deletedTrade)}</p>
      <p className="mt-1 text-xs text-slate-400">Undo is available for 10 seconds.</p>
